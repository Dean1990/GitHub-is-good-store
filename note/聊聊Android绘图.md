...
画完才发现，这不是股票图吧，应该是那种吧，那种一个条，两头尖的，叫K线图。

研究一下，这个名叫蜡烛的东西，需要4个值，开盘，收盘…姑且用一个数组表示吧，1234。
...
由于Android的坐标轴是从左上角为(0,0)点，Y轴和笛卡尔坐标系相反，所以就有transformYScale函数用于转换。

这个方式实现的蜡烛不能使用之前用到过的invalidate()方法，因为这次的path并不是一个整体的，当再次触发onDraw方式时，画板重置，之前的path绘制就被清除了，使用循环来处理这个问题。为什么不用一个整体的path？因为画笔（paint）的颜色在不断的变化。

由于需要画一个矩形，所以画笔（paint）的样式设置成填充+描边
